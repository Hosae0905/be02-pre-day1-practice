...
public class MiddleQuickSort {
    ...
    private int partition(int[] arr, int left, int right) {
        ...
        while (true) {
            ...
            while (arr[left] < pivot) {  // 왼쪽에 위치한 값이 피벗 값보다 커질 때까지 반복한다.
                ++left;      // 왼쪽 위치를 오른쪽으로 한 칸 이동시킨다.
            }
            // high를 줄이면서 high번째 배열요소가 pivot보다 작은 경우를 찾는 과정
            while (arr[right] > pivot && left <= right) {  // low위치가 high 위치보다 작거나 같으면 교차를 시켜야 되서 high 값을 빼줘야 함
                --right;     // 오른쪽 위치를 왼쪽으로 한 칸 이동시킨다.
            }

            // low가 high보다 크거나 같을 때
            if (left >= right) {  // low와 high가 만나거나 교차했을 경우
                return right;    // high 값을 반환
            }
